"""
evaluate.py — Full evaluation pipeline for the pretrained GPT model
─────────────────────────────────────────────────────────────────────
Generates N stories from held-out TinyStories prompts and scores them
against the corresponding references with BLEU, ROUGE, BERTScore, MAUVE,
Distinct-n, repetition rate, vocabulary coverage, and perplexity.

Usage (from the repo root):
    python scripts/evaluate.py --ckpt ckpt_best.pt --n_stories 500
    python scripts/evaluate.py --ckpt ckpt_best.pt --n_stories 20 --skip_bertscore --skip_mauve
"""

import os, re, sys, csv, math, argparse
import torch
import numpy as np
...
# (box-drawing and check-mark characters are used in the report below).
...
def compute_repetition(token_ids_list, n=4):
    """
    Average n-gram repetition rate across all generated stories.
...
def main(args):
    ...
    print(f"  Generated {args.n_stories}/{args.n_stories} stories done        \n")
...
if __name__ == "__main__":
    ...
    p.add_argument("--ckpt",           default="ckpt_best.pt",   help="Path to checkpoint")
